 ...
 * sense of scale. This means that models exported from different programs
 * might be very different sizes. If your model isn't displaying, try calling
 * loadModel() with the normalized parameter set to true. This will resize the
 * model to a scale appropriate for p5. You can also make additional changes to
 * the final size of your model with the scale() function.
 * 
 * @param {string} path Path of the model to be loaded
 * @param {boolean} normalize If true, scale the model to a standardized size
 * 		when loading
 * @param {(model: p5.Geometry) => void} successCallback Function to be called
 * 		once the model is loaded. Will be passed the 3D model object.
 * @param {(response: any) => void} failureCallback called with event error if
 * 		the image fails to load.
 */
declare function loadModel(path: string, normalize?: boolean,
	successCallback?: (model: p5.Geometry) => void,
	failureCallback?: (response: any) => void): p5.Geometry;
 ...
 * string must resemble a number, or NaN (not a number) will be returned. For
 ...
 * floats, it's necessary to first convert them to Strings using nf() or nfs().
 * 
 * @param {string[]} list array of Strings to be joined
 ...
 * expression, then the contents of each will be returned in the array. Element
 * [0] of a regular expression match returns the entire matching string, and
 ...
 * is null, then the sequence did not match at all. If the sequence did match,
 ...
 */
declare function matchAll(str: string, reg: string): string[];

/**
 * Utility function for formatting numbers into strings. There are two
 * versions: one for formatting floats, and one for formatting ints. The values
 * for the digits, left, and right parameters should always be positive
 * integers.
 * 
 ...
 * commas to mark units of 1000. There are two versions: one for formatting
 * ints, and one for formatting an array of ints. The value for the right
 * parameter should always be a positive integer.
 * 
 * @param {number} num the Number to format
 * @param {number} right number of digits to the right of the decimal point
 */
 ...
 * formatting ints. The values for left, and right parameters should always be
 * positive integers.
 * 
 * @param {number} num the Number to format
 * @param {number} left number of digits to the left of the decimal point
 ...
 * negative numbers. There are two versions: one for formatting floats, and one
 * for formatting ints. The values for the digits, left, and right parameters
...
/**
 * The split() function maps to String.split(), it breaks a String into pieces
 * using a character or string as the delimiter. The delim parameter specifies
 * the character or characters that mark the boundaries between each piece. A
 * String[] array is returned that contains each of the pieces.
 * 
 * The splitTokens() function works in a similar fashion, except that it splits
 * using a range of characters instead of a specific character or sequence.
 * 
 ...
 * 
 * @param {string} str a String or Array of Strings to be trimmed
 */
